...
async function mintNFTs(collectionAddress: string, quantity: number, maxETHperToken: number) {
  try {
    console.log(`Starting to mint ${quantity} NFTs from collection ${collectionAddress}`);
    console.log(`Maximum ETH per token: ${maxETHperToken}`);

    // Prepare mint transaction
    await reservoirClient.actions.mintToken({
      items: [{ collection: collectionAddress, quantity }],
      wallet: walletClient,
      onProgress: steps => {
        console.log(steps);
      },
...
async function main() {
  ...
  try {
    await mintNFTs(collectionAddress, quantityToMint, maxETHperToken);
  } catch (error) {
